<script lang="ts">
    ...
    import IconButton from "nunui/IconButton";
    import List, {OneLine} from "nunui/List";
    import CircularProgress from "nunui/CircularProgress";
    ...
    let expandable, expand = false, over = null, _over = null, _exp = false, open = false, anim = false;
    let overI = -1, _overI = -1;
...
<nav class:leftNav={$leftNav} class:topNav={!$leftNav} style:--height={$navHeight + 'px'}
    ...
    <div class="menu">
        ...
        {/if}
        <a class="logo" href="/" on:mouseenter={() => _overI = -1}>
        </a>
        {#if $leftNav}
            {#each links as link, i}
                <a href={link.path} on:click={(e)=>{
                    if(link.handler) {
                        e.preventDefault();
                        link.handler();
                    }
                }}>
                    <IconButton flat icon={link.icon} label={link.text} on:mouseenter={() => _overI = i}
                                active={_over === link || $page.url.pathname === link.path || $page.url.pathname.startsWith(link.path + '/')}/>
                </a>
            {/each}
        {/if}
        {#if !$leftNav}
        ...
        <List>
            {#each links as link}
                ...
                <a href={path} on:click={(e)=>{
                    ...
                    <OneLine title={text} icon={icon} round
                             active={active || _over === link || $page.url.pathname === path || $page.url.pathname.startsWith(path + '/')}/>
                </a>
            {/each}
        </List>
    </div>
            ...
            <List>
                {#each over.extra as {text, icon, path, handler, active}}
                    <a href={path} on:click={(e)=>{
                        ...
                        <OneLine title={text} {icon} round
                                 active={active|| $page.url.pathname === path || $page.url.pathname.startsWith(path + '/')}/>
                    </a>
                {/each}
            </List>
        </div>
